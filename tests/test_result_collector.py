...
def test_collect_missing_wrapper_name(result_collector, mock_proc_info):
    """Тест сбора результатов без имени wrapper"""
    del mock_proc_info["wrapper_name"]
    ...
    assert result["wrapper_name"] == "unknown wrapper"
    assert result["exit_code"] == 0
...
def test_collect_multiple_processes(result_collector, mock_proc_info):
    ...
    mock_proc_info_2["wrapper_name"] = "test_wrapper_2"
    ...
    assert result_collector.results[0]["wrapper_name"] == "test_wrapper"
    assert result_collector.results[0]["exit_code"] == 0
    assert result_collector.results[1]["wrapper_name"] == "test_wrapper_2"
...
@patch('builtins.print')
def test_final_report_with_results(mock_print, result_collector, mock_proc_info):
